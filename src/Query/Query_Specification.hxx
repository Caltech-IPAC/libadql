...
namespace
{
class Query_Specification_Columns_Visitor : public boost::static_visitor<std::ostream &>
{
public:
  std::ostream &os;
  ...
  std::ostream &operator()(const std::vector<ADQL::Query_Specification::Column_Variant> &s)
      const
  {
    auto o=s.begin ();
    if (o!=s.end ())
...
inline std::ostream &operator<<(std::ostream &os,
                                const ADQL::Query_Specification::Columns &columns)
{
  return boost::apply_visitor (Query_Specification_Columns_Visitor (os), columns);
}
...
BOOST_FUSION_ADAPT_STRUCT (
    ...
    (std::string, all_or_distinct)(size_t, top)(ADQL::Query_Specification::Columns,
                                                columns)
